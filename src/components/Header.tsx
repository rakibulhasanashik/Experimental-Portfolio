    ...
    <header className={`py-3 sm:py-4 fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'backdrop-blur-md bg-portfolio-bg/80 shadow-lg shadow-black/10' : 'bg-transparent'}`}>
      <div className="content-container flex justify-between items-center">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <button 
            onClick={toggleTheme} 
            className="relative h-8 w-[56px] xs:w-[56px] sm:w-[56px] rounded-full flex items-center justify-center overflow-hidden transition-all duration-300"
          ...
          <button 
            className="bg-[#6a0dad] clean-shine-button h-8 w-fit px-4 rounded-full hover:bg-opacity-80 transition-all duration-300 hover:shadow-[0_0_15px_rgba(139,92,246,0.5)] font-medium text-xs sm:text-sm text-white h-8"
